<template>
    <div class="choose-course container">
      <el-card class="form-card">
        ...
        <div class="search-bar">
          ...
          <el-button type="primary" @click="handleSearch" :loading="loading">搜索</el-button> <el-button @click="resetSearch" :disabled="loading">重置</el-button>
        </div>
      ...
      <el-empty v-if="showEmpty" description="暂无可选课程或未搜索到结果（请确认培养方案中是否有课程）" /> </div>
  </template>
  ...
  const fetchCourses = async () => {
    if (!studentId.value) { // Check if injected studentId is available
    ...
    try {
      ...
      // studentId is now directly used from inject
  ...
  const submitCourses = async () => { 
    if (!studentId.value) { // Check injected studentId before submitting
      ...
      for (const sectionId of selectedSectionIds) { // MODIFIED: Iterate over sectionId 
        try {
          const response = await submitStudentCourseSelection({
            student_id: Number(studentId.value), // Use injected studentId
